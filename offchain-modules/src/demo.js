...
const mint = async () => {
    ...
    const userCells = (await ckb.loadCells({ indexer, CellCollector, lock: userLockscript })).filter(cell => cell.data === '0x')
    ...
    const bridgeCells = (await ckb.loadCells({ indexer, CellCollector, lock: bridgeCellLockscript })).filter(cell => cell.data === '0x')
    console.dir({ bridgeCells }, {depth: null})

    // compose tx
    const inputCells = userCells.concat(bridgeCells[0])
    const inputs = inputCells.map(cell => { return { previousOutput: cell.outPoint, since: '0x0' }})
...
const burn = async (sudtCell) => {
    ...
    const rawTx = {
        ...
        outputsData: [
            bigintToSudtAmount(99),
            bigintToSudtAmount(1),
        ],
    }
    console.dir({ rawTx }, { depth: null })
    const signedTx = ckb.signTransaction(PRI_KEY)(rawTx)
