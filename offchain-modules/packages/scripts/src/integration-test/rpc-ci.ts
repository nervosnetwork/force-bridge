...
async function lock(testcases) {
  ...
  for (let i = 0; i < casesLength; i++) {
    ...
    if (testcase.send) {
      ...
      await checkTx(testcase.payload.asset.ident, lockTxHash, CKB_TEST_ADDRESS, ETH_TEST_ADDRESS);

      for (let j = 0; j < 3; j++) {
        const afterBalance = await getBalance(testcase.payload.asset.ident, CKB_TEST_ADDRESS, ETH_TEST_ADDRESS);
...
async function burn(testcases) {
  ...
  for (let i = 0; i < casesLength; i++) {
    ...
    if (testcase.send) {
      const beforeBalance = await getBalance(testcase.payload.asset, CKB_TEST_ADDRESS, ETH_TEST_ADDRESS);
      ...
      await checkTx(testcase.payload.asset, burnTxHash, CKB_TEST_ADDRESS, ETH_TEST_ADDRESS);

      const afterBalance = await getBalance(testcase.payload.asset, CKB_TEST_ADDRESS, ETH_TEST_ADDRESS);
      assert(
...
async function txSummaries(testcases) {
  const casesLength = testcases.length;
  for (let i = 0; i < casesLength; i++) {
    const testcase = testcases[i];
...
async function balance(testcases) {
  const casesLength = testcases.length;
  for (let i = 0; i < casesLength; i++) {
    const testcase = testcases[i];
    let balanceResult;
...
async function fee(method, testcases) {
  const casesLength = testcases.length;
  for (let i = 0; i < casesLength; i++) {
    const testcase = testcases[i];
    try {
...
async function getTransaction(token_address, userNetwork, address) {
  const getTxPayload = {
    network: 'Ethereum',
...
async function getFee(network, xchainAssetIdent, amount) {
  const payload = {
    network,
    xchainAssetIdent,
    amount,
...
async function getBalance(token_address, ckbAddress, ethAddress) {
...
async function checkTx(token_address, txId, ckbAddress, ethAddress) {
  let find = false;
  let pending = false;
  for (let i = 0; i < 2000; i++) {
    const txs = await getTransaction(token_address, 'Nervos', ckbAddress);
    for (const tx of txs) {
  ...
  find = false;
  pending = false;
  const txs = await getTransaction(token_address, 'Ethereum', ethAddress);
  for (const tx of txs) {
    if (tx.status == 'Successful' && tx.txSummary.fromTransaction.txId == txId) {
      logger.info('tx', tx);
      find = true;
...
async function main() {
  ...
  await txSummaries(txSummaryCases);
  await balance(balanceCases);
  await fee('getBridgeInNervosBridgeFee', feeCases);
  await fee('getBridgeOutNervosBridgeFee', feeCases);
}
