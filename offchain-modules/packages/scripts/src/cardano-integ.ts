...
function getBridgeAddr(config: MultisigConfig): string {
  ...
  for (let k of config.verifiers) {
    keyHashes.push(CardanoWasm.Ed25519KeyHash.from_bech32(k.adaPubkeyHash));
  }
  let bridgeMultiSigScript = utils.createMultiSigScript(keyHashes, config.threshold);
  let bridgeMultiSigAddr = utils.getScriptAddress(bridgeMultiSigScript, utils.cardanoTestnetNetworkId());
...
async function main() {
  ...
  const { ckbDeps, ownerConfig, multisigConfig, ckbStartHeight } =
    await deployDev(
      CKB_RPC_URL,
      CKB_INDEXER_URL,
      MULTISIG_NUMBER,
  ...
  // Wait for collector to register unlock Txs
