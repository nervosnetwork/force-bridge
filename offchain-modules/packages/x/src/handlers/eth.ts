  ...
  async watchNewBlock(): Promise<void> {
    ...
    foreverPromise(
      async () => {
        ...
        const actualBlockHeight = await this.ethChain.getCurrentBlockNumber();
        BridgeMetricSingleton.getInstance(this.role).setBlockHeightMetrics('eth', currentHeight, actualBlockHeight);
