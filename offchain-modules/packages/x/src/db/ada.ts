...
export interface AdaLatestTxHashTime {
  lastSeenTxTime: string | undefined,
  lastSeenTxHash: string | undefined,
}
...
export class AdaDb implements IQuery {
  ...
  async getLatestBlockTime(): Promise<AdaLatestTxHashTime> {
    const rawRes = await this.connection.manager.query('select tx_time, tx_hash from ada_lock where block_number = (select max(block_number) from ada_lock)');
    if (rawRes.length == 0) {
      return { lastSeenTxTime: undefined, lastSeenTxHash: undefined};
    } else {
      return { lastSeenTxTime: rawRes[0].txTime, lastSeenTxHash: rawRes[0].txTime };
    }
  }
