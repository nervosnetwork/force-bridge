...
# Remove previous run's data
rm -rf example
rm -f "$SOCKET_DIR/node-bft1.sock"
rm -f "$SOCKET_DIR/node-bft2.sock"
rm -f "$SOCKET_DIR/node-pool1.sock"
...
# Increase the time to 2s, as the AdaHandler has 15s interval and signer would fail if it gets more than 10 blocks behind
sed -i 's/"slotLength": 0.2/"slotLength": 2/' scripts/byron-to-alonzo/mkfiles.sh
